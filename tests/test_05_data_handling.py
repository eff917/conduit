...
def test_list_data(login_logout_driver):
    ...
    article_list = login_logout_driver.find_elements_by_xpath(
        '//a[@class="preview-link"]/h1'
    )
...
def test_list_multi_page_data(login_logout_driver):
    ...
    for page in page_nr_list:
        page.click()
        sleep(2)
        actual_page = login_logout_driver.find_element_by_xpath(
...
def test_insert_data(login_logout_driver):
    new_article_link = login_logout_driver.find_element_by_xpath(
        '//a[@href="#/editor"]'
    )
    new_article_link.click()
    sleep(2)
    take_screenshot(login_logout_driver, "before_fill")
    title_field = login_logout_driver.find_element_by_xpath(
        '//input[@placeholder="Article Title"]'
    )
    about_field = login_logout_driver.find_element_by_xpath(
        '//input[@placeholder="What\'s this article about?"]'
    )
    article_field = login_logout_driver.find_element_by_xpath(
        '//textarea[@placeholder="Write your article (in markdown)"]'
    )
    tags_field = login_logout_driver.find_element_by_xpath(
        '//input[@class="ti-new-tag-input ti-valid"]'
    )
    publish_button = login_logout_driver.find_element_by_xpath(
        '//button[@type="submit"]'
    )
    ...
    actual_tags_list = login_logout_driver.find_elements_by_xpath(
...
def test_update_data(login_logout_driver):
    ...
    sleep(2)
    edit_button = login_logout_driver.find_element_by_xpath(
        '//a[@href="#/editor/test-article"]'
    )
    edit_button.click()
    sleep(2)
    title_field = login_logout_driver.find_element_by_xpath(
...
def test_delete_data(login_logout_driver):
    login_logout_driver.get(URL + "#/articles/test-article")
    sleep(2)
    delete_button = login_logout_driver.find_element_by_xpath(
